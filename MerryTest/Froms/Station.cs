...
namespace MerryTest.Froms
{
    public partial class Station : Form
    {
        ...
        private void Station_Load(object sender, EventArgs e)
        {
            ...
            TestitemEntity.ItemData.ForEach(item =>
            {
                ...
                case "btn_StartRun":
                    ...
                    Task.Run(() =>
                    {
                        ...
                        if (testitem.测试项目.Contains("|"))
                        {
                            ...
                            if (arr.Length == 3) SingleTest._AllDll.Run("Switch", "Run", new object[] { new object[] { $@"dllname=Switch&跳转继电器分={arr[2]}" } });

                            Thread.Sleep(Convert.ToInt32(arr[1]));//项目前延时
                        }
                        bool flag = TestMethod.Test(testitem, out string testvalue);
                        Invoke(new Action(() =>
                        {
        ...
        {
            string path = $@".\TestItem\{SingleTest.Config["Name"]}";
            using (FileStream fs = new FileStream($"{path}\\{SingleTest.Config["Station"]}.txt", FileMode.Create, FileAccess.Write))
            {
